...
class RandomizedRPCA(BaseRPCA):
    def __init__(self, D, mu=None, lmbda=None):
        super().__init__(D, mu, lmbda)

    def svd_threshold(self, M, tau):
        rsvd = BaseRSVD(A, min(M.shape)//2, RandomizedRangeFinder, DirectSVD, q=1, debug=False)
        U, S, V = rsvd.U, rsvd.D, rsvd.Vh
        return np.dot(U, np.dot(np.diag(self.shrink(S, tau)), V))
